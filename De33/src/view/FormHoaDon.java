...
import controller33.IOFile;
import controller33.TrongException;
import java.io.File;
...
import model33.KhachHang;
import model33.DienThoai;
import model33.HoaDon;
...
public class FormHoaDon extends javax.swing.JPanel {
    ...
    private String fdthoai;
    private List<DienThoai> ldthoai;
    public FormHoaDon() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma KH","Ho ten KH","Ma SP","Hang","So luong"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        fHDon="src/controller/hoadon.dat";
        fdthoai="src/controller/dienthoai.dat";
        loadData();
        load2Ma();
    }
    ...
    private void loadData(){
        ...
        File fdt=new File(fdthoai);
        if(fdt.exists()){
            //List<Sach> list=IOFile.doc(fsach);
            ldthoai=IOFile.doc(fdthoai);
        }  
        else{
            ldthoai=new ArrayList<>();
        }
    }
    private void load2Ma(){
        ...
        String fkh="src/controller/khach hang.dat";
        ...
        if(l1!=null){
            ...
            for(DienThoai i:ldthoai)
                cbMaSP.addItem(i.getMa()+"");
        
    }
    private boolean trung2Ma(int makh, int masp){
        for(HoaDon i:lHDon)
            if(i.getKhachhang().getMa()==makh && i.getDienthoai().getMa()==masp)
                return true;
        return false;
    } 
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        try{
            ...
            gia=sl*getDTByMa(maSP).getGia();
            HoaDon hd=new HoaDon(getKHByMa(maKH),getDTByMa(maSP), sl, gia);
            tm.addRow(hd.toObjects());
            lHDon.add(hd);
            IOFile.viet(fdthoai, ldthoai);
    ...
    private DienThoai getDTByMa(int ma){
    ...
    private KhachHang getKHByMa(int ma){
        String fbdoc="src/controller/khach hang.dat";
