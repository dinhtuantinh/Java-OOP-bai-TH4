 ...
 * @author Admin
 */
public class FormBangKeKhai extends javax.swing.JPanel {

    DefaultTableModel tm;
    private String f1;
    private List<BangKKGD> l1;
    private String f2;
    private List<MonHoc> l2;
    public FormBangKeKhai() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma GV","Ten GV","Ma mon","ten mon","So tiet mon","So lop"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        f1="src/controller/QLGV.dat";
        f2="src/controller/MB.dat";
        loadData();
        load2Ma();
    }
    ...
    private void load2Ma(){
        ...
        String fbd="src/controller/GV.dat";
        List<GiangVien> l1=IOFile.doc(fbd);
        if(l1!=null){
            for(GiangVien i:l1)
...
//    }
    ...
    private void initComponents() {
        ...
        cbSapXep.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "...Lua chon sap xep...", "Ten giang vien", "So tiet day giam" }));
    ...
    private MonHoc getMByMa(int ma){
            for(MonHoc i:l2)
                if(i.getMa()==ma)
                    return i;
    ...
    private GiangVien getGVByMa(int ma){
        String f3="src/controller/GV.dat";
        List<GiangVien> l1=IOFile.doc(f3);
        if(l1!=null){
            for(GiangVien i:l1)
                if(i.getMa()==ma)
                    return i;
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        int sl=0;
        int slt=getMByMa(ma2).getStiet();
        try{
            sl=Integer.parseInt(jTextField1.getText());
            if(sl>3){
                JOptionPane.showMessageDialog(this, "Chi duc day toi da 3 lop");
                jTextField1.requestFocus();
                return;
            }
            if(sl*slt>200){
                JOptionPane.showMessageDialog(this, "Tong so tiet day cua cac lop phai nho hon 200");
                jTextField1.requestFocus();
                return;
            }
            int tien=0;
            int th=getMByMa(ma2).getStiet()-getMByMa(ma2).getLythuyet();
            tien=(int)(sl*getMByMa(ma2).getKinhphi()*(getMByMa(ma2).getLythuyet()+0.7*th));
            BangKKGD ql=new BangKKGD(getGVByMa(ma1),getMByMa(ma2), sl,tien);
    ...
    private void cbSapXepItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cbSapXepItemStateChanged
        ...
        if(cbSapXep.getSelectedIndex()==1){
            l1.sort(new Comparator<BangKKGD>(){
                @Override
                public int compare(BangKKGD o1, BangKKGD o2) {
        ...
        if(cbSapXep.getSelectedIndex()==2){
            l1.sort(new Comparator<BangKKGD>(){
                @Override
                public int compare(BangKKGD o1, BangKKGD o2) {
                   return Integer.compare(o2.getM().getStiet(), o1.getM().getStiet());
                }
