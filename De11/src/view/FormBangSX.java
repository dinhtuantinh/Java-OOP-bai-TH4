...
public class FormBangSX extends javax.swing.JPanel {
    ...
    public FormBangSX() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma KH","Ho ten","So phong can","Ma phong","Kieu phong",
            "so ngay"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        f1="src/controller/BANGSX.dat";
        f2="src/controller/PH.dat";
        loadData();
        load2Ma();
    }
    ...
    private int demDauPhong(int map){
        int count=0;
        for(BangSX i:l1)
            if(i.getP().getMa()==map)
                count++;
    ...
    private void initComponents() {
        ...
        cbSapXep.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "...Lua chon sap xep...", "Loai phong", "So phong can thue" }));
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        int sl=0;
        int slp=getPhongByMa(ma2).getSphong();
        
        try{
            sl=getKHByMa(ma1).getSluong();
            if(sl>slp){
                JOptionPane.showMessageDialog(this, "Khong du phong");
                cbMa2.requestFocus();
                return;
            }
            int sngay=Integer.parseInt(jTextField1.getText());
            int tien=getKHByMa(ma1).getSluong()*getPhongByMa(ma2).getGia();
            BangSX ql=new BangSX(getKHByMa(ma1),getPhongByMa(ma2), sngay, tien);
            tm.addRow(ql.toObjects());
            l1.add(ql);
            getPhongByMa(ma1).setSphong(slp-sl);
            IOFile.viet(f2, l2);
    ...
    private void cbSapXepItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cbSapXepItemStateChanged
        ...
        if(cbSapXep.getSelectedIndex()==1){
            l1.sort(new Comparator<BangSX>(){
                ...
                public int compare(BangSX o1, BangSX o2) {
                    String[] ten1=o1.getP().getKieu().split("\\s+");
                    String[] ten2=o2.getP().getKieu().split("\\s+");
        ...
        if(cbSapXep.getSelectedIndex()==2){
            l1.sort(new Comparator<BangSX>(){
                @Override
                public int compare(BangSX o1, BangSX o2) {
                    return Integer.compare(o1.getKh().getSluong(), o2.getKh().getSluong());
                }
