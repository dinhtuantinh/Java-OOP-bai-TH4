...
public class FormGiangVien extends javax.swing.JPanel {
    ...
    private String f;
    private List<GiangVien> l;
    //private List<Phong> l2;
    public FormGiangVien() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma Mon","Ten mon","So dien thoai","Khoa"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        buttonSate(true);
        them=false;
        sua=false;
        f="src/controller/GV.dat";
        loadData();
    }
    ...
    private void btCapNhatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btCapNhatActionPerformed
        ...
        if(them){
            ...
            try{
               ...
               if(ten.isEmpty()||sdt.isEmpty()) throw new TrongException();
               int ma=Integer.parseInt(jTextField1.getText());
               int dt=Integer.parseInt(jTextField3.getText());
               //if(loai.equalsIgnoreCase(getPhongByMa()))
               GiangVien b=new GiangVien(ma,ten,dt,jComboBox1.getSelectedItem().toString());
        ...
        if(sua){
            ...
            int dt=Integer.parseInt(tm.getValueAt(row, 2).toString());
            GiangVien b=new GiangVien(ma, ten, dt,khoa);
            tm.setValueAt(ma, row, 0);
            tm.setValueAt(ten, row, 1);
            tm.setValueAt(dt, row, 2);
            tm.setValueAt(khoa, row, 3);
            l.set(row, b);
            sua=false;
            buttonSate(true);
        }
    ...
    private void btThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btThemActionPerformed
        ...
        them=true;
        if(l!=null && l.size()>0)
            GiangVien.setSma(l.get(l.size()-1).getMa()+1);
        else
            l=new ArrayList<>();
    ...
    private void btSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSuaActionPerformed
        ...
        if(row<0 || row>jTable1.getRowCount()-1)
            JOptionPane.showMessageDialog(this, "Chon gv de sua");
        else{
            sua=true;
            buttonSate(false);
            jTextField2.requestFocus();
