...
public class FormBangDK extends javax.swing.JPanel {
    ...
    private String f1;
    private List<BangDK> l1;
    private String f2;
    private List<MonHoc> l2;
    public FormBangDK() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma GV","Ten GV","Ma mon","ten men",
            "so lop","thoi gian dk"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        f1="src/controller/QLDK.dat";
    ...
    private boolean trung2Ma(int mak, int mas){
        for(BangDK i:l1)
            if(i.getK().getMa()==mak && i.getM().getMa()==mas)
                return true;
        return false;
    } 
    private int demDauMon(int mas){
        int count=0;
        for(BangDK i:l1)
            if(i.getM().getMa()==mas)
                count++;
    ...
    private void initComponents() {
        ...
        cbSapXep.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "...Lua chon sap xep...", "Ten giang vien", "Thoi gian dang ky" }));
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        try{
            ...
            tg=jTextField2.getText();
            if(sl>6){
                JOptionPane.showMessageDialog(this, "Vuot qua so lop");
                jTextField1.requestFocus();
                return;
            }
            SimpleDateFormat df=new SimpleDateFormat("MM/dd/yyyy");
            Date date=df.parse(tg);
            BangDK ql=new BangDK(getGVByMa(ma1),getMonByMa(ma2), sl, date);
