...
public class FormMonHoc extends javax.swing.JPanel {
    ...
    private String fmon;
    private List<MonHoc> lmon;
    public FormMonHoc() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma mon","Ten mon","Tong so tiet","Loai mon hoc"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        buttonSate(true);
        them=false;
        sua=false;
        fmon="src/controller/MH.dat";
        loadData();
        
    }
    private void loadData(){
        File f=new File(fmon);
        if(f.exists()){
            //List<Sach> list=IOFile.doc(fsach);
            lmon=IOFile.doc(fmon);
        }  
        else{
            lmon=new ArrayList<>();
        }
    ...
    private void btCapNhatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btCapNhatActionPerformed
        ...
        if(them){
            ...
            try{
               ...
               sotiet=jTextField3.getText();
               if(ten.isEmpty()|| sotiet.isEmpty()) throw new TrongException();
               int st=Integer.parseInt(jTextField3.getText());
               if(st<0) throw new SoLuongException();
               int ma=Integer.parseInt(jTextField1.getText());
               MonHoc s=new MonHoc(ma,ten,st,jComboBox1.getSelectedItem().toString());
        ...
        if(sua){
            ...
            int st=Integer.parseInt(tm.getValueAt(row, 4).toString());
            try{
                st=Integer.parseInt(jTextField3.getText());
            }catch(NumberFormatException e){
                JOptionPane.showMessageDialog(this, "Nhap so");
            }
            MonHoc s=new MonHoc(ma, ten, st, loai);
            tm.setValueAt(ma, row, 0);
            tm.setValueAt(ten, row, 1);
            tm.setValueAt(st, row, 2);
    ...
    private void btThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btThemActionPerformed
        ...
        buttonSate(false);
        them=true;
        if(lmon!=null && lmon.size()>0)
            MonHoc.setSma(lmon.get(lmon.size()-1).getMa()+1);
        else
            lmon=new ArrayList<>();
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        ...
        if(row<0 || row>jTable1.getRowCount()-1)
            ...
            String loai=tm.getValueAt(row,4).toString();
