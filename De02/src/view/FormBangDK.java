 ...
 * @author Admin
 */
public class FormBangDK extends javax.swing.JPanel {

    DefaultTableModel tm;
    private String fdk;
    private List<BangDK> ldk;
    private String fmonhoc;
    private List<MonHoc> lmonhoc;
    public FormBangDK() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma sinh vien","Ho ten","Ma mon hoc","Ten mon hoc",
            "So tiet","Thoi gian"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        fdk="src/controller/QLDK.dat";
        fmonhoc="src/controller/MH.dat";
        loadData();
        load2Ma();
    }
    private void loadData(){
        ...
        File fs=new File(fmonhoc);
        if(fs.exists()){
            //List<Sach> list=IOFile.doc(fsach);
            lmonhoc=IOFile.doc(fmonhoc);
        }  
        else{
    ...
    private void load2Ma(){
        ...
        if(l1!=null){
            ...
            for(MonHoc i:lmonhoc)
                cbMamon.addItem(i.getMa()+"");
        
    }
    private boolean trung2Ma(int masv, int mam){
        for(BangDK i:ldk)
            if(i.getSv().getMa()==masv && i.getMon().getMa()==mam)
                return true;
        return false;
    } 
    private int demDauMon(int masv, int mam){
        int count=0;
        for(BangDK i:ldk)
            if(i.getSv().getMa()==masv&&i.getMon().getMa()==mam)
                count++;
    ...
    private void initComponents() {
        ...
        cbSapXep.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "...Lua chon sap xep...", "Ho ten sinh vien", "Thoi gian dang ky" }));
        ...
        cbMamon.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Item 1", "Item 2", "Item 3", "Item 4" }));
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        int mamon=Integer.parseInt(cbMamon.getSelectedItem().toString());
        if(trung2Ma(masv, mamon)){
            JOptionPane.showMessageDialog(this, "Trung 2 ma!");
            cbMasv.requestFocus();
            return;
        }
        if(demDauMon(masv, mamon)>8){
            JOptionPane.showMessageDialog(this, "Dang ky qua so mon!");
            cbMasv.requestFocus();
            return;
        }
        String tg;
        try{
            tg=jTextField1.getText();
            if(tg.isEmpty()) throw new TrongException();
            SimpleDateFormat df=new SimpleDateFormat("MM/dd/yyyy");
            Date date=df.parse(tg);
            BangDK ql=new BangDK(getSVByMa(masv),getMonByMa(mamon),date);
            tm.addRow(ql.toObjects());
            ldk.add(ql);
            
            IOFile.viet(fmonhoc, lmonhoc);
            ...
            JOptionPane.showMessageDialog(this, "Nhap dinh dang MM/dd/yyyy");;
        }
...
//            tm.addRow(i.toObjects());
    ...
    private void cbSapXepActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbSapXepActionPerformed
        ...
        if(cbSapXep.getSelectedIndex()==2){
            Collections.sort(ldk, (o1,o2)->{
                return o1.getTgian().compareTo(o2.getTgian());
            });
        }
        tm.setRowCount(0);
        for(BangDK i:ldk)
