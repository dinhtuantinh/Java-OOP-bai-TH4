 ...
 * @author Admin
 */
public class FormBangPC extends javax.swing.JPanel {

    DefaultTableModel tm;
    private String f1;
    private List<BangPC> l1;
    private String f2;
    private List<ViTri> l2;
    public FormBangPC() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma CT","Ten CT","Ma VT","ten VT","So tran"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        f1="src/controller/PCCT.dat";
        f2="src/controller/VITRI.dat";
        loadData();
        load2Ma();
    }
    ...
    private void load2Ma(){
        ...
        String fbd="src/controller/CT.dat";
        List<CauThu> l1=IOFile.doc(fbd);
        if(l1!=null){
            for(CauThu i:l1)
    ...
    private int demDauViTri(int mas){
        int count=0;
        for(BangPC i:l1)
            if(i.getM().getMa()==mas)
                count++;
    ...
    private void initComponents() {
        ...
        cbSapXep.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "...Lua chon sap xep...", "Ten cau thu", "So tran choi giam" }));
    ...
    private ViTri getVTByMa(int ma){
            for(ViTri i:l2)
                if(i.getMa()==ma)
                    return i;
    ...
    private CauThu getCTByMa(int ma){
        String f3="src/controller/CT.dat";
        List<CauThu> l1=IOFile.doc(f3);
        if(l1!=null){
            for(CauThu i:l1)
                if(i.getMa()==ma)
                    return i;
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if(demDauViTri(ma2)>3){
            JOptionPane.showMessageDialog(this, "Choi qua vi tri");
            cbMa1.requestFocus();
            return;
        }
        int st=0;
        int ml=getCTByMa(ma1).getMucl();
        int mt=getVTByMa(ma2).getMuc();
        try{
            st=Integer.parseInt(jTextField1.getText());
            int thuong=mt+(int)(0.1*ml);
            BangPC ql=new BangPC(getCTByMa(ma1),getVTByMa(ma2), st, thuong);
    ...
    private void cbSapXepItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cbSapXepItemStateChanged
        ...
        if(cbSapXep.getSelectedIndex()==1){
            l1.sort(new Comparator<BangPC>(){
                @Override
                public int compare(BangPC o1, BangPC o2) {
        ...
        if(cbSapXep.getSelectedIndex()==2){
            l1.sort(new Comparator<BangPC>(){
                @Override
                public int compare(BangPC o1, BangPC o2) {
                   return Integer.compare(o2.getSt(), o1.getSt());
                }
