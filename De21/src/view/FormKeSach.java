 ...
 * @author Admin
 */
public class FormKeSach extends javax.swing.JPanel {

    DefaultTableModel tm;
    private boolean them,sua;
    private String f;
    private List<KeSach> l;
    //private List<Phong> l2;
    public FormKeSach() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma Ke","Ten ke","Loai ke","So luong"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        buttonSate(true);
        them=false;
        sua=false;
        f="src/controller/KE.dat";
        loadData();
    }
    ...
    private void btCapNhatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btCapNhatActionPerformed
        ...
        if(them){
            ...
            try{
               ...
               loai=jTextField3.getText();
               sl=jTextField4.getText();
               if(ten.isEmpty()||loai.isEmpty()||sl.isEmpty()) throw new TrongException();
               int ma=Integer.parseInt(jTextField1.getText());
               int sluong=Integer.parseInt(jTextField4.getText());
               //if(loai.equalsIgnoreCase(getPhongByMa()))
               KeSach b=new KeSach(ma,ten,loai,sluong);
        ...
        if(sua){
            ...
            KeSach b=new KeSach(ma, ten, loai, sl);
            tm.setValueAt(ma, row, 0);
            tm.setValueAt(ten, row, 1);
            tm.setValueAt(loai, row, 2);
            tm.setValueAt(sl, row, 3);
            l.set(row, b);
            sua=false;
            buttonSate(true);
        }
    ...
    private void btSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSuaActionPerformed
        ...
        if(row<0 || row>jTable1.getRowCount()-1)
            JOptionPane.showMessageDialog(this, "Chon ke de sua");
        else{
            sua=true;
            buttonSate(false);
            jTextField2.requestFocus();
    ...
    private void btXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btXoaActionPerformed
        ...
        int row=jTable1.getSelectedRow();
        if(row<0 && row>jTable1.getRowCount()-1)
            JOptionPane.showMessageDialog(this, "Chon ke de xoa");
        else{
            tm.removeRow(row);
            l.remove(row);
        }
