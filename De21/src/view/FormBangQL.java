 ...
 * @author Admin
 */
public class FormBangQL extends javax.swing.JPanel {

    DefaultTableModel tm;
    private String f1;
    private List<BangQL> l1;
    private String f2;
    private List<Sach> l2;
    public FormBangQL() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma Ke","Ten ke","Ma sach","ten sach",
            "chuyen nganh","so luong sach","Tinh trang"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        f1="src/controller/QLSTV.dat";
        f2="src/controller/SACH.dat";
        loadData();
        load2Ma();
    }
    ...
    private void load2Ma(){
        ...
        String fbd="src/controller/KE.dat";
        List<KeSach> l1=IOFile.doc(fbd);
        if(l1!=null){
            for(KeSach i:l1)
    ...
    private boolean trung2Ma(int mak, int mas){
        for(BangQL i:l1)
            if(i.getK().getMa()==mak && i.getS().getMa()==mas)
                return true;
        return false;
    } 
    ...
    private void initComponents() {
        ...
        cbSapXep.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "...Lua chon sap xep...", "Ten sach", "So luong moi loai" }));
    ...
    private KeSach getKeByMa(int ma){
        String f3="src/controller/KE.dat";
        List<KeSach> l1=IOFile.doc(f3);
        if(l1!=null){
            for(KeSach i:l1)
                if(i.getMa()==ma)
                    return i;
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        int sl=0;
        int slp=getKeByMa(ma1).getSluong();
        
        try{
            sl=Integer.parseInt(jTextField1.getText());
            if(sl>slp){
                JOptionPane.showMessageDialog(this, "Khong du cho");
                jTextField1.requestFocus();
                return;
            }
            String tt=jTextField2.getText();
            BangQL ql=new BangQL(getKeByMa(ma1),getSachByMa(ma2), sl, tt);
            tm.addRow(ql.toObjects());
            l1.add(ql);
            getKeByMa(ma1).setSluong(slp-sl);
            IOFile.viet(f2, l2);
    ...
    private void cbSapXepItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cbSapXepItemStateChanged
        ...
        if(cbSapXep.getSelectedIndex()==1){
            l1.sort(new Comparator<BangQL>(){
                @Override
                public int compare(BangQL o1, BangQL o2) {
                    String[] ten1=o1.getS().getTen().split("\\s+");
                    String[] ten2=o2.getS().getTen().split("\\s+");
