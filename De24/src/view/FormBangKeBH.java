 ...
 * @author Admin
 */
public class FormBangKeBH extends javax.swing.JPanel {

    DefaultTableModel tm;
    private String f1;
    private List<BangKeBH> l1;
    private String f2;
    private List<MatHang> l2;
    public FormBangKeBH() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma KH","Ten KH","Ma hang","ten hang","nhom hang",
            "so luong"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        f1="src/controller/QLBH.dat";
...
//    }
    ...
    private void initComponents() {
        ...
        cbSapXep.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "...Lua chon sap xep...", "Ten khach hang", "Nhom mat hang" }));
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        try{
            ...
            int tien=sl*getMHByMa(ma2).getGia();
            BangKeBH ql=new BangKeBH(getKHByMa(ma1),getMHByMa(ma2), sl, tien);
            tm.addRow(ql.toObjects());
            l1.add(ql);
            getMHByMa(ma1).setSl(slh-sl);
            IOFile.viet(f2, l2);
    ...
    private void cbSapXepItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cbSapXepItemStateChanged
        ...
        if(cbSapXep.getSelectedIndex()==1){
            l1.sort(new Comparator<BangKeBH>(){
                @Override
                public int compare(BangKeBH o1, BangKeBH o2) {
        ...
        if(cbSapXep.getSelectedIndex()==2){
            l1.sort(new Comparator<BangKeBH>(){
                @Override
                public int compare(BangKeBH o1, BangKeBH o2) {
                    String[] hang1=o1.getM().getNhom().split("\\s+");
                    String[] hang2=o2.getM().getNhom().split("\\s+");
                    if(hang1[hang1.length-1].equalsIgnoreCase(hang2[hang2.length-1])){
                        return o1.getM().getNhom().compareToIgnoreCase(o2.getM().getNhom());
                    }
                    else
                        return hang1[hang1.length-1].compareToIgnoreCase(hang2[hang2.length-1]);
                }
