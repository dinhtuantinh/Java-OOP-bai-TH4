 ...
 * @author Admin
 */
public class FormTV extends javax.swing.JPanel {

    DefaultTableModel tm;
    private boolean them,sua;
    private String f;
    private List<ThanhVien> l;
    //private List<Phong> l2;
    public FormTV() {
        initComponents();
        setSize(950,850);
        String[] cols={"Ma TV","Ten TV","Tuoi","Muc luong"};
        tm=new DefaultTableModel(cols, 0);
        jTable1.setModel(tm);
        buttonSate(true);
        them=false;
        sua=false;
        f="src/controller/TV.dat";
        loadData();
    }
    ...
    private void btCapNhatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btCapNhatActionPerformed
        ...
        if(them){
            String ten,tuoi,mluong;
            try{
               ten=jTextField2.getText();
               tuoi=jTextField3.getText();
               mluong=jTextField4.getText();
               if(ten.isEmpty()||tuoi.isEmpty()||mluong.isEmpty()) throw new TrongException();
               int ma=Integer.parseInt(jTextField1.getText());
               int ml=Integer.parseInt(jTextField4.getText());
               ThanhVien b=new ThanhVien(ma,ten,tuoi,ml);
        ...
        if(sua){
            ...
            tuoi=jTextField3.getText();
            int ma=Integer.parseInt(jTextField1.getText());
            int ml=Integer.parseInt(tm.getValueAt(row, 3).toString());
            try{
                ml=Integer.parseInt(jTextField4.getText());
            }catch(NumberFormatException e){
                JOptionPane.showMessageDialog(this, "Nhap so");
            }
            ThanhVien b=new ThanhVien(ma, ten, tuoi, ml);
    ...
    private void btThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btThemActionPerformed
        ...
        them=true;
        if(l!=null && l.size()>0)
            ThanhVien.setSma(l.get(l.size()-1).getMa()+1);
        else
            l=new ArrayList<>();
        jTextField1.setText(ThanhVien.getSma()+"");
        jTextField2.requestFocus();
    ...
    private void btSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSuaActionPerformed
        ...
        if(row<0 || row>jTable1.getRowCount()-1)
            JOptionPane.showMessageDialog(this, "Chon tv de sua");
        else{
            sua=true;
            buttonSate(false);
            jTextField2.requestFocus();
    ...
    private void btXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btXoaActionPerformed
        ...
        int row=jTable1.getSelectedRow();
        if(row<0 && row>jTable1.getRowCount()-1)
            JOptionPane.showMessageDialog(this, "Chon tv de xoa");
        else{
            tm.removeRow(row);
            l.remove(row);
        }
